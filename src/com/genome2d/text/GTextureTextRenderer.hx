...
class GTextureTextRenderer extends GTextRenderer {
	...
	static public var fontScaleMultiplier:Float = 1;
    ...
    override public function render(p_x:Float, p_y:Float, p_scaleX:Float, p_scaleY:Float, p_rotation:Float, p_red:Float, p_green:Float, p_blue:Float, p_alpha:Float):Void {
		...
		var finalScale = g2d_fontScale * fontScaleMultiplier;
		...
		var scrollOffset:Float = scrollLine * (g2d_textureFont.lineHeight + g2d_lineSpace) * finalScale;
        ...
        for (i in 0...charCount) {
			...
			var cx:Float = renderable.x + renderable.xoffset*finalScale;
			var cy:Float = renderable.y + renderable.yoffset*finalScale - scrollOffset;
			...
			if (charRed == 1 && charBlue == 1 && charGreen == 1 && charAlpha == 1) {
				g2d_context.draw(renderable.texture, GBlendMode.NORMAL, tx, ty, p_scaleX * finalScale, p_scaleY * finalScale, p_rotation, red * p_red, green * p_green, blue * p_blue, alpha * p_alpha, null);
			} else {
				g2d_context.draw(renderable.texture, GBlendMode.NORMAL, tx, ty, p_scaleX * finalScale, p_scaleY * finalScale, p_rotation, charRed * p_red, charGreen * p_green, charBlue * p_blue, charAlpha * p_alpha, null);
			}
        }
	}
	...
	private function renderSelection(p_x:Float, p_y:Float, p_scaleX:Float, p_scaleY:Float, p_rotation:Float):Void {
		...
		if (cursorStartIndex == cursorEndIndex && Std.int(g2d_cursorBlinkCount / 10) % 2 == 0) {
			...
			if (g2d_chars.length > 0) {
				...
				tx = char.x * p_scaleX * finalScale + p_x + (cursorStartIndex>=g2d_chars.length?char.xadvance + g2d_tracking:0);
				ty = char.y * p_scaleY * finalScale + p_y;
			}
			var char:GTextureChar = g2d_textureFont.getChar(124);
			g2d_context.draw(char.texture, GBlendMode.NORMAL, tx, ty, p_scaleX * finalScale, p_scaleY * finalScale, p_rotation, red, green, blue, alpha, null);
			...
			var sx:Float = startChar.x * p_scaleX * finalScale + p_x + (cursorStartIndex >= g2d_chars.length?startChar.xadvance + g2d_tracking:0);
			var sy:Float = startChar.y * p_scaleY * finalScale + p_y;

			var endChar:GTextureCharRenderable = (cursorEndIndex >= g2d_chars.length) ? g2d_chars[g2d_chars.length - 1] : g2d_chars[cursorEndIndex];
			var ex:Float = endChar.x * p_scaleX * finalScale + p_x + (cursorEndIndex >= g2d_chars.length? endChar.xadvance + g2d_tracking:0);
			var ey:Float = endChar.y * p_scaleY * finalScale + p_y;

			if (sy == ey) {
				g2d_context.draw(g2d_helperTexture, GBlendMode.NORMAL, sx, sy, (ex-sx)/4*p_scaleX * finalScale, g2d_textureFont.lineHeight/4*p_scaleY * finalScale, p_rotation, 1, 1, 1, 1, null);
			} else {
				g2d_context.draw(g2d_helperTexture, GBlendMode.NORMAL, sx, sy, (g2d_width + p_x - sx) / 4 * p_scaleX * finalScale, g2d_textureFont.lineHeight / 4 * p_scaleY * finalScale, p_rotation, 1, 1, 1, 1, null);
				for (i in 1...Std.int((ey - sy) / g2d_textureFont.lineHeight)) {
					g2d_context.draw(g2d_helperTexture, GBlendMode.NORMAL, p_x, sy+i*g2d_textureFont.lineHeight, g2d_width / 4 * p_scaleX * finalScale, g2d_textureFont.lineHeight / 4 * p_scaleY * finalScale, p_rotation, 1, 1, 1, 1, null);
				}
				g2d_context.draw(g2d_helperTexture, GBlendMode.NORMAL, p_x, ey, (ex - p_x) / 4 * p_scaleX * finalScale, g2d_textureFont.lineHeight / 4 * p_scaleY * finalScale, p_rotation, 1, 1, 1, 1, null);
			}
		}
	}
    ...
    override public function invalidate():Void {
        ...
        while (i < g2d_textLength) {
            ...
            if (currentCharCode == 10 || currentCharCode == 13) {
                ...
                if (!g2d_autoSize && offsetY + 2 * (g2d_textureFont.lineHeight + g2d_lineSpace)*finalScale > g2d_height && isAllVisible) {
					isAllVisible = false;
					g2d_maxVisibleLine = lines.length - 1;
				}
				if (previousRenderable != null && previousRenderable.x + (previousRenderable.width * finalScale) > maxLineWidth) {
					maxLineWidth = previousRenderable.x + (previousRenderable.width * finalScale);
				}
                offsetX = 0;
                offsetY += (g2d_textureFont.lineHeight + g2d_lineSpace)*finalScale;
				...
				// TODO: Vertical autosize
                if (!g2d_autoSize && offsetY + (g2d_textureFont.lineHeight + g2d_lineSpace) * finalScale > g2d_height && isAllVisible) {
					isAllVisible = false;
					g2d_maxVisibleLine = lines.length - 1;
				}
                ...
                if (previousCharCode != -1) {
                    offsetX += g2d_textureFont.getKerning(previousCharCode, currentCharCode) * finalScale;
                }

				renderable.setCharCode(currentCharCode);

				if (!g2d_autoSize && offsetX + char.texture.width*finalScale > g2d_width) {
					if (!wordWrap) break;
					lines.push(currentLine);
                ...
                offsetX += (char.xadvance + g2d_tracking) * finalScale;
        ...
        if (g2d_autoSize) {
            ...
            g2d_height = offsetY + g2d_textureFont.lineHeight * finalScale;
        } else {
			g2d_textWidth = maxLineWidth;
		}
		g2d_textHeight = offsetY + g2d_textureFont.lineHeight * finalScale;

        var bottom:Float = g2d_maxVisibleLine * (g2d_textureFont.lineHeight + g2d_lineSpace) * finalScale + g2d_textureFont.lineHeight * finalScale;
        var offsetY:Float = 0;
        ...
        for (i in 0...lines.length) {
            ...
            var right:Float = last.x - last.xoffset * finalScale + last.xadvance * finalScale;
	...
	private function getCharAt(p_x:Float, p_y:Float):Int {
		...
		for (i in 0...charCount) {
			...
			var tx:Float = char.x * finalScale;
			var ty:Float = char.y * finalScale;

			var difX:Float = p_x - tx;
			if (difX < 0) continue;

			var difY:Float = p_y - ty;
			if (difY < -char.yoffset * finalScale) continue;

			if (difX < minX && difY < g2d_textureFont.lineHeight * finalScale) {
				minX = difX;
				minY = difY;
				minIndex = i;
			}
		...
		if (minIndex<charCount && minX > finalScale*g2d_chars[minIndex].width / 2) minIndex++;
