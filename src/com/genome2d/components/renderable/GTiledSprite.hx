    ...
    **/
    ...
    public function captureMouseInput(p_captured:Bool, p_cameraX:Float, p_cameraY:Float, p_contextInput:GMouseInput):Bool {
        ...
        if (tx >= 0 && tx <= 1 && ty >= 0 && ty <= 1) {
            node.dispatchMouseCallback(p_contextInput.type, node, tx*g2d_width, ty*g2d_height, p_contextInput);
            if (node.g2d_mouseOverNode != node) {
                node.dispatchMouseCallback(GMouseInputType.MOUSE_OVER, node, tx*g2d_width, ty*g2d_height, p_contextInput);
            }

            return true;
        } else {
            if (node.g2d_mouseOverNode == node) {
                node.dispatchMouseCallback(GMouseInputType.MOUSE_OUT, node, tx*g2d_width, ty*g2d_height, p_contextInput);
            }
        }
