    ...
    inline static private function generateGetPrototype() {
        return macro : {
             public function getPrototype(p_prototypeXml:Xml = null):Xml {
                ...
                if (PROTOTYPE_PROPERTY_NAMES != null) {
                    for (i in 0...PROTOTYPE_PROPERTY_NAMES.length) {
                        var name:String = PROTOTYPE_PROPERTY_NAMES[i];
                        if (PROTOTYPE_PROPERTY_TYPES[i] == "IGPrototypable") {
                            var xml:Xml = Xml.createElement(name);
                            var property:com.genome2d.proto.IGPrototypable = cast Reflect.getProperty(this, name);
                            if (property != null) {
                                xml.addChild(property.getPrototype());
                                p_prototypeXml.addChild(xml);
                            }
                        } else {
                            p_prototypeXml.set(name,Std.string(Reflect.getProperty(this, name)));
                        }
                    }
                }
    ...
    inline static private function generateInitPrototype() {
        return macro : {
            public function initPrototype(p_prototypeXml:Xml):Void {
                ...
                for (i in 0...PROTOTYPE_PROPERTY_NAMES.length) {
                    var name:String = PROTOTYPE_PROPERTY_NAMES[i];
                    var type:String = PROTOTYPE_PROPERTY_TYPES[i];
                    ...
                    if (realValue != null) {
                        try {
                            Reflect.setProperty(this, name, realValue);
                        } catch (e:Dynamic) {
                        }
                    }
